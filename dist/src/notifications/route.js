...
server.post('/api/notification/send', async (req, res) => {
    try {
        const { to, title, body, data, sound, priority, channelId } = await req.body;
        ...
        if (result.data) {
            const successCount = result.data.filter((ticket) => ticket.status === 'ok').length;
            const errorCount = result.data.filter((ticket) => ticket.status === 'error').length;
