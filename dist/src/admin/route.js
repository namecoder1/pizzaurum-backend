...
server.post('/api/admin/create-rider', async (req, res) => {
    try {
        ...
        if (authCheckError)
            throw httpError(500, 'Error fetching users', authCheckError.message);
        ...
        if (existingAuthUser) {
            ...
            if (updateError)
                throw httpError(500, 'Error updating user', updateError.message);
            return {
                success: true,
                message: `Rider ${name} created successfully`,
                user: {
                    id: existingAuthUser === null || existingAuthUser === void 0 ? void 0 : existingAuthUser.id,
                    email,
                    name
                },
        ...
        if (createError)
            throw httpError(500, 'Error creating user', createError.message);
        ...
        if (insertError)
            throw httpError(500, 'Error inserting user', insertError.message);
        ...
        if (emailError)
            throw httpError(500, 'Error sending invite email', emailError.message);
        return {
            success: true,
            message: `Rider ${name} created successfully`,
            user: {
...
server.post('/api/admin/create-admin', async (req, res) => {
    try {
        ...
        if (existingAuthUser) {
            ...
            if (updateError)
                throw httpError(400, "Error updating existing user:", updateError.message);
            return {
                success: true,
        ...
        if (createError)
            throw httpError(400, 'Erro creating admin user:', createError.message);
        const { error: insertError } = await admin
            .from('users')
            .upsert({
            id: userData.user.id,
        ...
        if (emailError)
            console.log('Error sending invite email:', emailError.message);
        return {
            success: true,
...
server.post('/api/admin/change-password', async (req, res) => {
    try {
        const admin = supabaseAdmin;
        const { userId, currentPassword, newPassword } = await req.body;
        ...
        if (weakPasswords.includes(newPassword.toLowerCase()))
            throw httpError(400, 'The password is too common, avoid classic patterns and retry.');
        ...
        if (updateError)
            throw httpError(400, 'Error changing the password:', updateError.message);
        return {
            success: true,
