...
import { sendOrderStatusPushNotification } from "../../lib/notifications.js";
...
server.post('/api/orders/assign-rider', async (req, res) => {
    try {
        ...
        const { orderId, riderId } = await req.body;
        ...
        const { data: orderCheck, error: checkError } = await admin
            ...
            .single();
        if (checkError)
            throw httpError(404, 'Error checking order', checkError.message);
        if (!orderCheck)
            throw httpError(404, 'Order not found');
        if (orderCheck.driver_id && orderCheck.driver_id !== riderId)
            throw httpError(409, 'Order already assigned to another rider');
        const { data: updatedOrder, error: updateError } = await admin
            ...
            .select();
        if (updateError)
            throw httpError(500, 'Error updating order', updateError.message);
        if (!updatedOrder || updatedOrder.length === 0)
            throw httpError(500, 'Order assignment failed');
        return {
            success: true,
            data: updatedOrder[0],
...
server.post('/api/orders/complete-order', async (req, res) => {
    try {
        ...
        if (!orderCheck)
            throw httpError(404, 'Order not found');
        if (orderCheck.driver_id !== riderId)
            throw httpError(403, 'Order is not assigned to this rider');
        if (orderCheck.status !== 'delivering')
            throw httpError(400, 'Order must be in delivering status to be completed');
        const { data: completedOrder, error: completedError } = await admin
            ...
            .select();
        if (completedError)
            throw httpError(500, 'Error updating the order:', completedError.message);
        if (!completedOrder || completedOrder.length === 0)
            throw httpError(500, 'Order completion failed');
...
server.post('/api/orders/update-status', async (req, res) => {
    try {
        const admin = supabaseAdmin;
        const { orderId, newStatus } = await req.body;
        ...
        const { data: orderData, error: orderError } = await admin
            ...
            .single();
        if (orderError)
            throw httpError(500, 'Error fetching order', orderError.message);
        ...
        const shouldSendSMS = ((orderData.is_delivery === false && newStatus === 'ready_to_pickup') ||
            (orderData.is_delivery === true && newStatus === 'delivering'));
        if (shouldSendSMS && (userData === null || userData === void 0 ? void 0 : userData.phone)) {
            try {
                let message = '';
        ...
        if (userData === null || userData === void 0 ? void 0 : userData.expo_push_token) {
            try {
