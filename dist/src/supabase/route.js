...
const getAccessToken = (authorization) => authorization === null || authorization === void 0 ? void 0 : authorization.replace("Bearer ", "");
const getUserClient = async (authorization) => {
    const token = getAccessToken(authorization);
    if (!token)
        throw httpError(401, "Missing access token");
    const { data: authData, error: authError } = await supabaseAdmin.auth.getUser(token);
    if (authError || !(authData === null || authData === void 0 ? void 0 : authData.user))
        throw httpError(401, "Invalid or expired access token", authError === null || authError === void 0 ? void 0 : authError.message);
...
const requireAdmin = async (authorization) => {
    ...
    const { data: roleData, error: roleError } = await supabaseAdmin
        ...
        .single();
    if (roleError)
        throw httpError(500, 'Error checking user role', roleError.message);
    if ((roleData === null || roleData === void 0 ? void 0 : roleData.role) !== 'admin')
...
server.get('/api/orders', async (req, res) => {
    try {
        ...
        const { data, error } = await supabaseUser
            ...
            .select('*');
        if (error)
            throw httpError(500, 'Error fetching orders', error.message);
        return data;
    }
    catch (error) {
...
server.get('/api/admin/orders', async (req, res) => {
    try {
        ...
        const { data, error } = await supabaseAdmin
            ...
            .select('*');
        if (error)
            throw httpError(500, 'Error fetching all orders', error.message);
        return {
            success: true,
            data,
...
server.get('/api/orders/:id', async (req, res) => {
    try {
        ...
        const { data: orderData, error: fetchError } = await supabaseUser
            ...
            .single();
        if (fetchError)
            throw httpError(500, 'Fetch error', fetchError.message);
        return {
            success: true,
            data: orderData,
...
server.post('/api/orders', async (req, res) => {
    try {
        ...
        const { data, userId: requestedUserId } = await req.body;
        ...
        const { data: createdOrder, error: creatingError } = await supabaseUser
            ...
            .single();
        if (creatingError)
            throw httpError(500, 'Error creating the order', creatingError.message);
        return {
            success: true,
            data: createdOrder,
