...
import { CURRENCY } from "../../utils/types.js";
import { handleCheckoutSessionCompleted, handlePaymentIntentSucceeded, handlePaymentIntentFailed, handleInvoicePaymentSucceeded, handleChargeFailed, handleChargeUpdated, handleInvoicePaymentFailed, formatAmountForStripe, getStripeFeesAndPaymentMethod } from "../../utils/stripe.js";
dotenv.config();
...
server.post('/api/stripe/create-checkout', async (req, res) => {
    try {
        const body = await req.body;
        const { items, totalPrice, user_id, user_email, is_delivery, customer_time, is_custom_time, stripe_customer_id } = body;
        ...
        const total_extras = products
            .reduce((sum, p) => { var _a; return sum + (((_a = p.extras) === null || _a === void 0 ? void 0 : _a.length) || 0); }, 0)
            .toString();
        const products_extras = products
            ...
            if (extras.length === 0)
                return null;
            return `${index}:${extras.map((e) => `${e.name}|${e.price}`).join(',')}`;
        })
            .filter(Boolean)
            .join(';');
...
server.post('/api/stripe/create-payment-sheet', async (req, res) => {
    try {
        const { amount, metadata, cartItems, user_id, user_email, stripe_customer_id } = await req.body;
        ...
        const ephemeralKey = await stripe.ephemeralKeys.create({ customer: customer.id }, { apiVersion: "2025-06-30.basil" });
        ...
        const products_extras = products
            ...
            .filter(Boolean)
            .join(';');
        const { items } = metadata, metadataWithoutItems = __rest(metadata, ["items"]);
        const enhancedMetadata = Object.assign(Object.assign({}, metadataWithoutItems), { 
            // Compact format: "id:qty:extras_count,id:qty:extras_count"
            products_compact: products_compact, total_extras: total_extras, 
            // Extras format: "0:name|price,name|price;1:name|price"
            products_extras: products_extras });
        ...
        if (!stripe_customer_id && user_id && customer.id) {
            try {
                ...
                const { error: userUpdateError } = await admin
                    ...
                    .eq("id", user_id);
                if (userUpdateError)
                    throw httpError(500, 'Error updating user with Stripe customer ID', userUpdateError.message);
            }
            catch (userError) {
                throw httpError(500, 'Error updating user', (userError === null || userError === void 0 ? void 0 : userError.message) || userError);
            }
        }
        const response = {
...
server.get('/api/stripe/order-id', async (req, res) => {
    try {
        ...
        const { data: order, error } = await admin
            ...
            .single();
        if (error)
            throw httpError(500, 'Error retrieving order', error.message);
...
server.post('/api/stripe/payment-methods', async (req, res) => {
    ...
    try {
        ...
        const grouped = lastCharges.data.reduce((acc, charge) => {
        ...
        const updatedPayments = lastPayments.data.map((payment) => {
            const relatedCharges = allCharges.data.filter((charge) => charge.payment_intent === payment.id);
            ...
            if (hasRefunds) {
                return Object.assign(Object.assign({}, payment), { status: 'refunded', refunded: true });
            }
            return payment;
        });
        lastPayments.data = updatedPayments;
        chargesWithoutPaymentIntent = chargesWithoutPaymentIntent.map((charge) => {
            if (charge.refunded || (charge.refunds && charge.refunds.data && charge.refunds.data.length > 0)) {
                return Object.assign(Object.assign({}, charge), { status: 'refunded' });
            }
            return charge;
        });
        // Query Supabase per trovare gli ordini associati ai payment intents
        const paymentIntentIds = lastPayments.data.map((payment) => {
        ...
        if (ordersError)
            throw httpError(500, 'Error retrieving orders', ordersError.message);
        ...
        const paymentsWithOrders = lastPayments.data.map((payment) => {
            ...
            return Object.assign(Object.assign({}, payment), { order: orderData || null });
        });
        const paymentSummary = {
            totalSpent: lastCharges.data.reduce((sum, charge) => (charge.status === 'succeeded' && !charge.refunded) ? sum + charge.amount / 100 : sum, 0),
            successfulPayments: lastPayments.data.filter((p) => p.status === 'succeeded' && !p.refunded).length,
            refundedPayments: lastPayments.data.filter((p) => p.refunded || p.status === 'refunded').length,
            lastPaymentDate: (lastPayments.data.length > 0 && ((_a = lastPayments.data[0]) === null || _a === void 0 ? void 0 : _a.created) !== undefined) ?
...
server.post('/api/stripe/refund', async (req, res) => {
    var _a;
    try {
        const { orderId, userId, isAdmin } = await req.body;
        ...
        const { data: order, error: fetchError } = await admin
            ...
            .single();
        if (fetchError || !order)
            throw httpError(404, 'Error fetching order', (fetchError === null || fetchError === void 0 ? void 0 : fetchError.message) || 'Order not found.');
        const { data: user, error: userError } = await admin
            ...
            .single();
        if (userError || !user)
            throw httpError(404, 'Error fetching user', (userError === null || userError === void 0 ? void 0 : userError.message) || 'User not found.');
        if (!isAdmin) {
        ...
        const { error: updateError } = await admin
            ...
            .eq("id", orderId);
        if (updateError)
            throw httpError(500, 'Error updating order status', updateError.message);
        const { error: updateUserError } = await admin
            .from('users')
            .update({
            reputation_score: Number((_a = user === null || user === void 0 ? void 0 : user.reputation_score) !== null && _a !== void 0 ? _a : 0) - 1
        })
            .eq("id", order.user_id);
        if (updateUserError)
            throw httpError(500, 'Error updating user reputation', updateUserError.message);
        return {
            success: true,
            refund_id: refund.id,
...
server.post('/api/stripe/update-net-profit', async (req, res) => {
    try {
        ...
        const { data, error } = await admin
            ...
            .single();
        if (error)
            throw httpError(500, 'Error updating order net profit', error.message);
        return {
            success: true,
            orderId,
            paymentIntentId,
